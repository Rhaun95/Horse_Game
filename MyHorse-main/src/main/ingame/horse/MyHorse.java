...
public class MyHorse {
	...
	//마구간
	ArrayList<MyHorse> myStall = new ArrayList<MyHorse>();
	...
	//내 말들 보관함(용량 2) 받아오기
	public ArrayList<MyHorse> getMyStall(){
		return this.myStall;
	}
...
//	말 객체 생성(name, color)
	public void createHorse() {
		...
		if(myStall.isEmpty()) {
			...
			c = sc.nextLine();
			myStall.add(new MyHorse(n, c));
			
			System.out.println("========축하합니다 [" + myStall.get(0).getName() + "]가 생성되었습니다!========");
			print(myStall);
			System.out.println("===================================================");
		}else {
	...
	public void addToRaceHorses(MyHorse h) {
			r.getRaceStall().add(0, new RaceHorse(h.getName(), h.getSpeed(),h.getColor()));
	}		

	//전체조회(1~8)
	public void print(ArrayList<MyHorse> myStall) {
		
		for(MyHorse horse : myStall) {
